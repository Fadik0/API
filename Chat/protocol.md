...
//req_to_server
{
    type: "join",
    data: {
        channel_id: "5" || "r123" // идентификатор канала или или если начинается с префикса "r" id-комнаты
...
//res_to_client
{
    ...
    data: {
        ...
        premium_only: 0, // режим в чате только для премиум подписчиков
        started: "1451202121", // время запуска стрима, unix timestamp
        room_privacy: 0, 
        room_role: 0,
        premium: true,
        premiums:[],
 ...
 * /join room_id  - сервер отошлет запрос на подключение к комнате с указанным room_id
 * /motd message - устанавливает в сообщение дня значение message
 * /smiles 1 | 0 - включение, отключение смайлов в чате
 * /slowmod 1 | 0 - включение, отключение задержки в чате
...
//res_to_all_in_channel
{
    ...
    data: {
        ...
        premium: 0,   // премиум статус пользователя отправившего сообщение
        premiums: [],
        hideIcon: 0,  // используется в служебных целях на стороне клиента
...
### При изменение настроек чата сервер оповещает всех сообщением на канале:

Следующие inline-команды изменяют настройки
 * /motd message - устанавливает в сообщение дня значение message
 * /smiles 1 | 0 - включение, отключение смайлов в чате
 * /slowmod 1 | 0 - включение, отключение задержки в чате
...
res_to_all_in_channel
{
    ...
    data: {
        channel_id: "5",
        name: "motd", // название настройки, может быть motd, smiles, slowmod
        value:"123", // значение настройки, motd - аналоговый текст, smiles и slowmod - 1 или 0 (true или false)
        moder_id:90496, 
        moder_name:"Fadiko",
        moder_rights: 20,
        moder_premium: 0,
...
| streamer | Стример | 20 |
| referee | Судья турнира | 22 |
| sponsor | Организатор турнира | 25 |
...
Уровни доната для смайлов можно узнать из собщений `success_join`, `message`, `private_message` поле `payments`:
